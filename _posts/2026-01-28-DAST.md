...
$$\mathcal{L}_{gra} = \sum_{i,j} \left( \left\| \left|(\hat{I}_t)_{i,j} - (\hat{I}_t)_{i-1,j}\right| - \left|(I_t)_{i,j} - (I_t)_{i-1,j}\right| \right\|_1 + \left\| \left|(\hat{I}_t)_{i,j} - (\hat{I}_t)_{i,j-1}\right| - \left|(I_t)_{i,j} - (I_t)_{i,j-1}\right| \right\|_1 \right)$$

위 두 가지 손실을 가중치($\lambda$)를 적용하여 합친 최종 손실 함수를 구한다. 

$$\mathcal{L} = \lambda_1 \mathcal{L}_{int} + \lambda_2 \mathcal{L}_{gra}$$
